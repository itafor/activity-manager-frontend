...
import ServiceProviderInfoTab from './ServiceProviderInfoTabs'
import { getOneClient } from '../../redux/clientSlice'
import { getOneServiceProviders } from '../../redux/serviceProviderSlice'

const ClientInfo = () => {
  const dispatch = useDispatch()
  const navigate = useNavigate()
  const { id } = useParams()
  const { singleData, loading } = useSelector((state) => state.serviceProviders)

  useEffect(() => {
    dispatch(getOneServiceProviders(id))
  }, [id])
    ...
    <StyledContainer>
      <div>
        <div className='userInfo'>
          ...
          <ServiceProviderInfoTab className='infoTab' />
        </div>
      </div>
    </StyledContainer>
  )
